...
public class CustomerService {
    ...
    public CustomerResponse getCustomerByDni(String dni) throws IdNotFoundException {
        Optional<CustomerEntity> customerExist = this.customerRepository.findCustomerEntitiesByIdentityCardNumber(dni);
        if (!customerExist.isPresent()) {
            throw new IdNotFoundException("the id: "+dni+" was not found");
        }else{
            CustomerEntity customer = customerExist.get();
            return new CustomerResponse(customer.getId(),customer.getFirstName(),
                    customer.getLastName(), customer.getHomeAddress(),
    ...
    public CustomerResponse deleteById(int id) throws IdNotFoundException {
        Optional<CustomerEntity> customerExist = this.customerRepository.findById(id);
        if (!customerExist.isPresent()) {
            throw new IdNotFoundException("the id was not found");
        }
        else{
            this.customerRepository.delete(customerExist.get());
            return new CustomerResponse(customerExist.get().getId(),
                    customerExist.get().getFirstName(),
                    customerExist.get().getLastName(),
                    customerExist.get().getHomeAddress(),
                    customerExist.get().getIdentityCardNumber(),
                    customerExist.get().getDateBirth(),
                    customerExist.get().getPhoneNumber()
                    ,customerExist.get().getEmail());
        }
    }
}
